...
def single_conv_lay_3d(input, num_filters, filter_size, batch_norm=False, dropout=False, activation='relu'):
    """ Adds a single convolutional layer with the specifiec parameters. It can add batch normalization and dropout
        ...
        :return:
    """
    conv1 = Conv3D(num_filters, (filter_size, filter_size, filter_size), padding='same', activation=activation)(input)
...
def multiple_conv_lay_3d(input, num_filters, filter_size, make_pool=True, batch_norm=False, dropout=False, tot_layers=2, activation='relu'):
    ...
    for c_layer_idx in range(tot_layers):
        c_input = single_conv_lay_3d(c_input, num_filters, filter_size, batch_norm=batch_norm, dropout=dropout, activation=activation)
...
def make_multistream_3d_unet(inputs, num_filters=8, filter_size=3, num_levels=3,
    ...
    [conv_p, dele] = multiple_conv_lay_3d(merged, filters, filter_size, make_pool=False, batch_norm=batch_norm_decoding,
                                          dropout=dropout_decodign, activation=activation)
    print(F"Filters {filters} Conv (before deconv): {conv_p.shape}")
    conv_t = UpSampling3D((2, 2, 2))(conv_p)
    ...
    for level in range(1,num_levels+1):
        ...
        [conv_p, dele] = multiple_conv_lay_3d(merged, filters, filter_size, make_pool=False,
                                              batch_norm=batch_norm_decoding, dropout=dropout_decodign, activation=activation)
        print(F"Filters {filters} Conv (before deconv): {conv_p.shape}")
    ...
    last_conv = Conv3D(num_output_bands, (1, 1, 1), activation=last_activation)(conv_p)
...
def make_inverted_multistream_3d_unet(inputs, tot_streams, num_filters, filter_size, num_levels):
    ...
    for c_stream in range(tot_streams):
        print(F"----------- Stream {c_stream} ----------- ")
        last_input_stream = UpSampling3D((2, 2, 2))(conv_p)
        for level in range(1, num_levels+1):
            bottom_up_level = num_levels-level
            filters = num_filters * (2 ** (bottom_up_level))
            ...
            print(F"Filters {filters} Conv (before deconv): {conv_p.shape}")
            if level != (num_levels):
                last_input_stream = UpSampling3D((2, 2, 2))(last_input_stream)
                print(F"Filters {filters} Conv (after deconv): {conv_t.shape}")
